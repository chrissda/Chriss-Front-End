 ...
 * 2. resolver el algoritmo - etapa de manejo de datos, me refiero 
 * que habrá algun algoritmo,
 ...
 * Objetivo: Mostrar una lista de tareas con lo que tengo en 
 * el html mediante el manejo del DOM
 * 2.1 Necesito obtener el ingreso del usuario dentro del input
 * 2.2 Necesito la accion del button, el click del button
...
//sleector que acepta nomenclatura de CSS
...
function dibujarTareas(){
    let htmlTareas = "";
    //estamos aniadiendo codigo html o htmlTareas
...
function guardarTareaLS(){
    //recordemos, local storage(LS) solo me permite texto
    let listaTareasJSON = JSON.stringify(listaTareas);
    localStorage.setItem("todo", listaTareasJSON);
}

function leerTareasLS(){
    //cuando obtenemos el valor de LS con getItem 
    //lo obtenemos como string
...
btnAgregar.addEventListener("click", function(){
    ...
    //push es un método que me permite aniadir algo a un arreglo
...
leerTareasLS();
//en caso deseen borrar todo el LS, puede utilizar el metodo LocalStorage




/* Primera version del ejercicio
let tareas = [];
