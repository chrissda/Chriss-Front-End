/**
 * 0.Objetivo
 * 1. de donde obntengas los datos
...
btnTarea.addEventListener('click', function () {
    //Todo es un objeto en JS, incluido los elementos que obtiene del DOM
    //al ser objetos tendran propiedades y métodos útiles
    console.log(inputTarea.value);
    //Agregarlo en data.tareas
    ...
    // ------- probando
    dataTareas.forEach(function(item){
        let elTarea = ItemTarea(item);
...
function ItemTarea(task, index){
    ...
    //es un atributo de tipo data y lo que haces es guardar informacion
    li.innerHTML = `
        ...
        <input 
            ...
            >
    `
    //querySelector obtiene un elemento usando la sintaxis de CSS . #
...
function dibujarTareas(elemTarea){
    ...
    //lo limpia como su fuera una pizarra
    //ul.innerHTML = "";
    //agrega un nuevo elemento al ul
